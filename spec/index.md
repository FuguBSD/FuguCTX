# FuguCTX specification

FuguCTX is a configuration repair model for OpenBSD daemons, built as the pilot
of FuguTTX. The engine reads a broken daemon configuration and a parser error,
and it proposes the smallest fix as a unified diff. The build rehearses the
FuguTTX production pipeline at small scale, on the same components, at real
prices. Cheap learnings are a deliverable, and the
[learning register](register.md) records them.
...
- A citation of a unit of a sibling repository is a prose token with the
  repository name in front, for example FuguTTX TRN-TRACES. It is never a link,
  and it never names a plan. Write the repository name before every cited ID, in
  tables included.
- A marker of the form "Rehearses: FuguTTX TRN-TRACES" ties a component to the
...
| Code | Document                               | Area                                                             |
| ---- | -------------------------------------- | ---------------------------------------------------------------- |
| ENG  | [engine.md](engine.md)                 | The repair engine, the checker table, and its interfaces         |
